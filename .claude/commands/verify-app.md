...
4. **Use-case assets** — run `/check-use-case-assets`. Report every missing file, mirror-parity gap, naming violation, and folder mismatch found.

## Reporting

After all checks:
- If everything passes: state clearly "All checks passed — typecheck, lint, tests, and asset integrity are green."
...
- Never skip a check even if a previous one failed — run all four and report all issues at once so they can be fixed together.
- The asset check (step 4) is read-only and non-blocking for TypeScript/lint/test failures, but must pass before the task is marked complete.
